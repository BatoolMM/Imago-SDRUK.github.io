...
export const permissionCreateUseCase = async ({
	data,
	session,
	configuration,
	...
	const [errs, permissions] = await authorisation_module.createPermission(schema)
	if (errs !== null) {
		return err(errs)
	}
...
export const permissionsCreateUseCase = async ({
	data,
	session,
	configuration,
	authorisation_module
